        ...
        >
          {resumeData ? (
            <div className="space-y-2">
              ...
              {renderListSection("Skills", Object.keys(resumeData.skills || {}))}
              {renderListSection(
                "Work Experience",
                resumeData.workExperience?.map(
                  (exp) => `${exp.jobTitle} at ${exp.company} (${exp.startDate} - ${exp.endDate})`
                )
              )}
              {renderListSection(
                "Education",
                resumeData.education?.map(
                  (edu) => `${edu.degree} in ${edu.major} from ${edu.university} (${edu.startDate} - ${edu.endDate})`
                )
              )}
              {renderListSection(
                "Projects",
                resumeData.projects?.map(
                  (proj) => `${proj.name}: ${proj.description} (${proj.link})`
                )
              )}
              {renderListSection(
                "Certifications",
              ...
              {renderSection("Achievements", resumeData.achievements)}
            </div>
          ) : parsedData ? (
            <div className="space-y-2">
            ...
            <p>Resume data will be shown here.</p>
          )}
        </motion.div>
