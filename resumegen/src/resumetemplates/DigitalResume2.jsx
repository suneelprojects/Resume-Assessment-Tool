...
const DigitalResume2 = () => {
  ...
  const parseHTML = (htmlString) => {
    ...
    // Modified parseAchievements function to safely handle different data types
    const parseAchievements = (achievements) => {
      if (!achievements) return [];
      
      // If achievements is already an array, return it filtered
      if (Array.isArray(achievements)) {
        return achievements.filter(item => item && item.trim() !== '');
      }
      
      // If achievements is a string but not HTML
      if (typeof achievements === 'string' && !achievements.includes('<')) {
        return achievements.split('\n').filter(item => item && item.trim() !== '');
      }
      
      // If achievements is HTML string
      if (typeof achievements === 'string' && achievements.includes('<')) {
        try {
          const parser = new DOMParser();
          ...
          // If HTML parsing fails, split by newlines as fallback
          return achievements.split('\n').filter(item => item && item.trim() !== '');
        }
      }
