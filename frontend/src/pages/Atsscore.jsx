...
const Atsscore = () => {
  ...
  const [resumeText, setResumeText] = useState(""); // Automatically populated resume text
  ...
  const handleDomainChange = (e) => {
    ...
    setRole(""); // Reset role selection when domain changes
  };
  ...
  const handleAnalyzeClick = async () => {
    ...
    const apiUrl = backendUrl.includes(":5000")
      ? `${backendUrl}/api/calculate_ats`
      : `${backendUrl}:5000/api/calculate_ats`;
    ...
    <div className="bg-gradient-to-br from-indigo-500 via-purple-500 to-pink-500 min-h-screen flex justify-center items-center py-10">
      <div className="container max-w-4xl mx-auto px-6 py-8 bg-white rounded-3xl shadow-xl">
        ...
        <div className="space-y-8">
          ...
          {/* Domain and Role Selection Side by Side */}
          <motion.div
            initial={{ opacity: 0, y: 30 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.8 }}
