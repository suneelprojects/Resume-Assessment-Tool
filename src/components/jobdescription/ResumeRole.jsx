...
const ResumeRole = () => {
    ...
    const [parsedData, setParsedData] = useState(null);  // To store parsedData
    ...
    // Reference for the prediction results container
    const predictionResultRef = useRef(null);

    useEffect(() => {
        if (location.state) {
            setResumeText(location.state.resumeText);  // Set extracted text
            setParsedData(location.state.parsedData);  // Set parsed data
        }
    }, [location.state]);

    // Trigger scroll to prediction results when predictionResult changes
    useEffect(() => {
        if (predictionResult) {
        ...
        <div className="min-h-screen flex flex-col justify-center items-center pt-16 relative overflow-hidden bg-gradient-to-r from-teal-400 to-indigo-500">
          <div className="container max-w-7xl mx-auto p-8 flex flex-col md:flex-row gap-12 items-start justify-center relative mt-8 space-y-12 md:space-y-0">
            ...
            {predictionResult && !loading && (
                <div
                    ref={predictionResultRef} // Attach the ref to the container
                ...
                >
                    ...
                    >
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                            {predictionResult.suggested_roles.map((suggestedRole, index) => (
                                <motion.div
                                    key={index}
